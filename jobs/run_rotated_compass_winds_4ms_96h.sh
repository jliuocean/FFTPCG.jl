...
log_dir="${project_root}/jobs/logs/rotated_compass_winds_4ms_96h"
mkdir -p "${log_dir}"
...
for direction in "${directions[@]}"; do
    ...
    echo "Starting ${direction} wind experiment at 4 m/s; log: ${log_file}"
    env JULIA_LOAD_PATH="${HOME}/Projects/Oceananigans.jl:${HOME}/Projects/FFTPCG.jl/env/h200:@stdlib" \
        ...
        --pickup false \
        >"${log_file}" 2>&1
    echo "Completed ${direction} wind experiment at 4 m/s"
done

echo "Completed all eight 4 m/s compass-wind experiments"
