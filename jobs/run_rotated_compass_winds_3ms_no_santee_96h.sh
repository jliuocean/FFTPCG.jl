...
log_dir="${project_root}/jobs/logs/rotated_compass_winds_3ms_no_santee_96h"
mkdir -p "${log_dir}"
...
for direction in "${directions[@]}"; do
    ...
    echo "Starting ${direction}: no Santee River, Winyah 375 m3/s, wind 3 m/s"
    env JULIA_LOAD_PATH="${HOME}/Projects/Oceananigans.jl:${HOME}/Projects/FFTPCG.jl/env/h200:@stdlib" \
        ...
        --pickup false \
        >"${log_file}" 2>&1
    echo "Completed ${direction} no-Santee experiment"
done

echo "Completed all eight 3 m/s no-Santee experiments"
