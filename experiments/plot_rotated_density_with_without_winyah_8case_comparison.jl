...
with_winyah = Dict(case => FieldTimeSeries(case_file(WITH_WINYAH_PREFIX, case), "b")
                   for case in CASES)
without_winyah = Dict(case => FieldTimeSeries(case_file(WITHOUT_WINYAH_PREFIX, case), "b")
                      for case in CASES)
...
@inline function in_original_winyah_jetty(x, y, no_winyah)
    no_winyah && return false
...
@inline function in_original_winyah_channel(x, y, no_winyah)
    return !no_winyah &&
...
@inline function original_shelf_depth(x, y, no_winyah)
    if y > original_river_mouth_y
        in_embayment = in_original_santee_embayment(x) ||
                       (!no_winyah && in_original_winyah_embayment(x))
...
@inline function original_water_depth(x, y, no_winyah)
    in_original_winyah_jetty(x, y, no_winyah) && return 0.0
    in_original_winyah_channel(x, y, no_winyah) && return inlet_depth
    in_original_santee_channel(x, y) && return inlet_depth
    return original_shelf_depth(x, y, no_winyah)
end

@inline water_depth(x, y, no_winyah) = original_water_depth(y, -x, no_winyah)
wet_with_winyah = [water_depth(x, y, false) > 0 &&
                   z_surface >= -water_depth(x, y, false) for x in x_m, y in y_m]
wet_without_winyah = [water_depth(x, y, true) > 0 &&
...
n = Observable(1)
with_surface = Dict(case => lift(nn -> density_surface(with_winyah[case][nn],
                                                       wet_with_winyah), n)
                    for case in CASES)
without_surface = Dict(case => lift(nn -> density_surface(without_winyah[case][nn],
                                                          wet_without_winyah), n)
                       for case in CASES)

fig = Figure(size = (2300, 2100), fontsize = 16, backgroundcolor = :white)
Label(fig[0, 1:3],
      "Surface density anomaly: influence of removing Winyah Bay",
      fontsize = 28)
...
for case in CASES
    ...
    # Draw the grid after the density field so it remains visible above it.
    for ax in (ax_with, ax_without)
...
@info "Saved with-vs-without-Winyah surface density comparison" output_file
